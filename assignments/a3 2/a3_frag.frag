...
{	
	...
	vec3 d = vtx_color.rgb * max(dot(n, normalize(lightDir)), 0.0) * 0.75;
	vec3 a = vtx_color.rgb * 0.2;
	vec3 s = vec3(0.2, 0.2, 0.2) * pow(max(dot(n, normalize(lightDir + normalize(position.xyz - worldPos.xyz))), 0.0), 32.0);
